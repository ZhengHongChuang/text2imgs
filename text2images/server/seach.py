...
import numpy as np
# 从自定义模块导入特定组件
from utils.convert_precision import preprocess_text
...
def retrieve_images(query):
    ...
    image_feat_path = Path(__file__).parent.parent / "databases/img_feat.jsonl"
    tsv_file_path = Path(__file__).parent.parent / "databases/imgs.tsv"
    ...
    model = load_model()
    with torch.no_grad():
        query = tokenize(preprocess_text(str(query)))
